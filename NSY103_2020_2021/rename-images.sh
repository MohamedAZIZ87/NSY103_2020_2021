	...
	then
		...
		for image in ` find $folder -type f `; 
			...
			do
				...
				ext=$(exiftool "$image" | grep -w "File Type Extension" | awk -F ": " '{print $2}')
				...
				gps=$(exiftool "$image" | grep -w "GPS Position")
					if [ -n "$gps" ];
						then
							...
							# Ici on va extraire les chiffre trouve dans le nom de la fichier pour la renommage. #
							chiff=$(exiftool "$image" | grep "File Name" | tr -dc '0-9')

							# La reference dans les donnees xml et le place exactement ou pris la photo. #
							ref1=$( echo "$xmldata"|grep -o -P '(?<=ref=").*(?=" lat)' | sed -e "s/[,'.]//g")
							
							# Cete ligne est pour convertir les lettre accente vers des lettres normaux. #
							ref2=$(echo "$ref1" | iconv -f utf8 -t ascii//translit)
							...
							if [[ "$ref2" =~ "?" ]]; 
								then 
									ref=$(echo "$ref1" | sed -e "s/ \+/_/g")
								else
									ref=$(echo "$ref2" | sed "s/-//g; s/ \+/_/g")
							fi
							...
							# Ici on va renommer le photo par la reference "lieu de photo si existe" plus les chiffre qui ont etaient deja dans le nom de photo. #
							name="$ref""_""$chiff"

								# Si la reference existe renomme l'image et deplace le dans le dossier sinon garde la meme nom et deplace le quandmeme dans le dossier.#
								if [ -z "$ref" ]; 
									then 
										mv -v "$image" "$pays/$ville/$type/$date/$basename" 2>>trifoto.log
									else 
										mv -v "$image" "$pays/$ville/$type/$date/$name.$ext" 2>>trifoto.log
								fi
